...
import { Save, Send, CheckCircle2, AlertCircle, Trophy, Star } from "lucide-react";
      ...
      <main className="max-w-7xl mx-auto px-6 mt-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {submissions.map((item, i) => (
            ...
            >
              <div className="relative aspect-[4/5] overflow-hidden rounded-[2.5rem] bg-zinc-900 border border-white/5 group-hover:border-purple-500/50 transition-all duration-500 shadow-2xl">
                <img
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end">
                  ...
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Candidate Entry</span>
                  </div>

                  <div className="bg-white/10 backdrop-blur-md border border-white/10 p-5 rounded-[2rem] space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-black text-white/50 uppercase tracking-widest">Score</span>
                      <span className="text-2xl font-black text-white">{scores[item._id] ?? 0}</span>
                    </div>
                    ...
                    <input
                      ...
                      className="w-full h-1 bg-white/20 rounded-lg appearance-none cursor-pointer accent-white disabled:opacity-30 transition-all"
                    />
                  </div>
                </div>
              </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[100] px-6">
            ...
            >
              ...
              <p className="text-zinc-500 mb-8 text-sm leading-relaxed font-medium">
                Once finalized, scores are locked. You will not be able to modify them further.
              </p>

              <div className="grid grid-cols-2 gap-4">
                <button
                  onClick={() => setShowModal(false)}
                  className="py-4 rounded-2xl border border-zinc-800 hover:bg-zinc-800 transition-colors font-bold text-zinc-400 text-sm"
                >
                  Cancel
                </button>
                <button
                  onClick={handleFinalSubmit}
                  className="py-4 rounded-2xl bg-white text-black hover:bg-zinc-200 transition-colors font-bold text-sm"
                >
                  Finalize
                </button>
              </div>
