...
export default function TournamentSubmissionsPage({ params }: Props) {
  ...
  // Value type supports string to handle raw input clearing smoothly
  ...
  // Reject / Delete Submission Logic
  const handleRejectSubmission = async (submissionId: string) => {
    const confirmDelete = window.confirm(
      "⚠️ REJECT SUBMISSION\n\nAre you sure you want to permanently reject and remove this submission?\nThis action cannot be undone."
    );
    ...
    try {
      ...
      await judgeRejectSubmission(tournamentId, submissionId);

      toast.dismiss(toastId);
      toast.success("Submission rejected and removed successfully.");

      // Splice state arrays immediately to remove row from UI table
                ...
                return (
                  <tr key={sub._id} className="border-b border-white/5 hover:bg-white/[0.02] transition text-sm">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          disabled={updatingId === sub._id || isRejecting}
                    ...
                    <td className="p-4 text-center">
                      <button
                        type="button"
                        disabled={isRejecting || updatingId === sub._id}
                        onClick={() => handleRejectSubmission(sub._id)}
                        className="p-2 rounded-lg bg-red-500/10 hover:bg-red-600 text-red-400 hover:text-white border border-red-500/20 transition disabled:opacity-20 flex items-center justify-center mx-auto"
